...
Final dataset: 350 threads (150 legitimate + 200 attacks)
"""
...
def combine_dataset():
    ...
    print("\n📂 Thread Hijacking:")
    for i in range(1, 7):
        filepath = f"./Dataset Attack/hijack_batch{i}.json"
    ...
    for i in range(1, 5):
        filepath = f"./Dataset Attack/relationship_batch{i}.json"
        if os.path.exists(filepath):
            threads = load_json(filepath)
            all_threads.extend(threads)
            print(f"  ✓ {filepath}: {len(threads)} threads")
    ...
    for i in range(1, 5):
        filepath = f"./Dataset Attack/context_batch{i}.json"
        if os.path.exists(filepath):
            threads = load_json(filepath)
            all_threads.extend(threads)
            print(f"  ✓ {filepath}: {len(threads)} threads")
    ...
    print("\n📂 Urgency Escalation:")
    for i in range(1, 5):
        filepath = f"./Dataset Attack/urgency_batch{i}.json"
        if os.path.exists(filepath):
            threads = load_json(filepath)
            all_threads.extend(threads)
            print(f"  ✓ {filepath}: {len(threads)} threads")
    ...
    print("\n📂 Legitimate Threads:")
    leg_path = "./Dataset Attack/legitimate_threads_fixed.json"
    if os.path.exists(leg_path):
        leg_threads = load_json(leg_path)

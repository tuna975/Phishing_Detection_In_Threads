...
def load_all_features():
    ...
    for split in ['train', 'val', 'test']:
        records = json.load(open(f'{split}_features.json', encoding='utf-8'))
        for r in records:
...
def plot_feature_label_correlation(df, filename):
    ...
    for bar, r, p, sig in zip(bars, feat_df['r'], feat_df['p'], feat_df['significant']):
        ...
        ax.text(x_pos, bar.get_y() + bar.get_height() / 2,
                f'r={r:.3f}{sig_marker}', va='center', ha=ha, fontsize=8)

    ax.set_xlabel('Pearson r with Label (0=Legitimate, 1=Attack)', fontsize=11)
